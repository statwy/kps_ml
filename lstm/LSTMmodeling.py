...
def buildModel(nInput):
    model = Sequential()
    model.add(LSTM(10, input_shape=(8,nInput)))
    model.add(Dense(1))
...
def TrainDataSet(data, prior=1):
    ...
    nLen = len(ft)
    n = int(nLen * 0.8) - 1
    trainX = ft.iloc[0:n,2:10].values
    trainY = ft.iloc[0:n,10].values
    testX = ft.iloc[n:(nLen-1),2:10].values
    testY = ft.iloc[n:(nLen-1),10].values
    trainX = np.reshape(trainX, (trainX.shape[0], 1, trainX.shape[1]))
    #trainX = np.reshape(trainX, (-1, 8, 10))    
...
#        df = npiSpread(x.codeA, x.codeB)
        ...
        #학습 데이터를 구성한다
        trainX, trainY,testX, testY = TrainDataSet(data, prior = nPrior)
...
data=pd.read_csv("data/inputdataset_final.csv")
simLearning(data)