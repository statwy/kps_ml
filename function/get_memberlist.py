...
def get_memberlist(predict_percent) :
    ...
    # SQL문 실행
    sql = "select address from contact where member_no in (select member_no from alarm where percent="+predict_percent+" and contact_no=1) and type=1 and certification=1"
    curs.execute(sql)
